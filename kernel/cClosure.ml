...
let rec resubst subs v =
  ...
  | FCaseInvert (ci, u, pms, p, indices, c, ve, env) ->
    ...
    { v with term = FCaseInvert (ci, u, pms, p, indices, c, ve, env) }
  | FFix (fx, e, g) ->
...
let rec knr info tab ~pat_state m stk =
  ...
  | FCaseInvert (ci, u, pms, (((_pnas, pbody), _r) as p),iv,c,v,env) when red_set info.i_flags fMATCH ->
    begin match case_inversion info tab ci u pms pbody iv v env with
